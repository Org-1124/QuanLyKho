...
group       by tblHangHoa.TenHang",nam);
        ...
        {
            string sTruyVan = string.Format(@"select Thang=MONTH(tblHoaDonNhap.NgayXuat) ,TongTien = sum( tblChiTietNhap.SoLuong* tblChiTietNhap.DonGia) from tblChiTietNhap, tblHoaDonNhap, tblHangHoa
            ...
            group by MONTH(tblHoaDonXuat.NgayNhap)", nam);
            con = DataProvider.KetNoi();
            DataTable dt = DataProvider.LayDataTable(sTruyVan, con);
            DataProvider.DongKetNoi(con);
            return dt;
